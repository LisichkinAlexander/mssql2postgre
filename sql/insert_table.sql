...
SELECT @SQL = 'INSERT INTO ' + @object_name + CHAR(13) + '(' + CHAR(13) + STUFF((
    SELECT CHAR(13) + '    , "' + c.name + '" ' 
    FROM sys.columns c WITH(NOLOCK)
    WHERE c.[object_id] = @object_id
    ORDER BY c.column_id
    FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 7, '      ') + CHAR(13) + ') select ' + CHAR(13) +
	STUFF((
    SELECT CHAR(13) + '    , unnest(cast( %(' + c.name + ')s as ' + 
                CASE 
